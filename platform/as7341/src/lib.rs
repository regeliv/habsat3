...
mod tests {
    ...
    #[test]
    fn test_as_bytes_f1_f5_nir() {
        let raw_bytes: [u8; 20] = [
            0x30, 0x01, 0x00, 0x00, 0x00, 0x42, 0x50, 0x00, 0x00, 0x50, 0x00, 0x00, 0x20, 0x04,
            0x00, 0x30, 0x01, 0x00, 0x00, 0x06,
        ];

        assert_eq!(PixelConnections::f1_f5_nir().as_bytes(), &raw_bytes);
    }

    #[test]
    fn test_as_bytes_f5_f8() {
        let raw_bytes: [u8; 20] = [
            0x00, 0x00, 0x00, 0x30, 0x01, 0x00, 0x00, 0x02, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00,
            0x13, 0x00, 0x00, 0x00, 0x00, 0x00,
        ];

        assert_eq!(PixelConnections::f6_f8().as_bytes(), &raw_bytes);
    }
